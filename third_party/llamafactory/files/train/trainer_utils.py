...
def _iter_expected_boxes(
    logits: "torch.Tensor",
    shift_labels: "torch.Tensor",
    coord_lo: int,
    coord_hi: int,
):
    r"""Yield ``(pred_box, target_box)`` in [0, 1] for every sample carrying a full bbox.

    The box is the FIRST four coordinate tokens of the sample, which is where ``bbox_1000``
    sits in the emitted JSON; the point coordinates that follow are ignored. Each predicted
    coordinate is decoded by EXPECTATION over the bins (the GFL/DFL integral).
    """
    n_bins = coord_hi - coord_lo + 1
    bins = torch.arange(n_bins, device=logits.device, dtype=logits.dtype)
    coord_mask = (shift_labels >= coord_lo) & (shift_labels <= coord_hi)

    for b in range(logits.size(0)):
        positions = coord_mask[b].nonzero(as_tuple=True)[0][:4]
        if positions.numel() < 4:
            continue

        probs = torch.softmax(logits[b, positions, coord_lo : coord_hi + 1], dim=-1)
        pred = (probs * bins).sum(dim=-1) / (n_bins - 1)
        target = (shift_labels[b, positions] - coord_lo).to(logits.dtype) / (n_bins - 1)
        yield pred, target


def expected_box_iou(
    logits: "torch.Tensor",
    shift_labels: "torch.Tensor",
    coord_lo: int,
    coord_hi: int,
) -> tuple[float, int]:
    r"""Mean plain IoU between the expectation-decoded boxes and their targets.

    Diagnostic only — computed under ``no_grad`` and never part of the loss. This is the
    number to watch during training: it says whether the boxes are actually landing on
    their targets, which a falling total loss does not.
    """
    ious = []
    with torch.no_grad():
        for pred, target in _iter_expected_boxes(logits, shift_labels, coord_lo, coord_hi):
            px1, px2 = torch.minimum(pred[0], pred[2]), torch.maximum(pred[0], pred[2])
            py1, py2 = torch.minimum(pred[1], pred[3]), torch.maximum(pred[1], pred[3])
            tx1, ty1, tx2, ty2 = target
            inter = (torch.clamp(torch.minimum(px2, tx2) - torch.maximum(px1, tx1), min=0)
                     * torch.clamp(torch.minimum(py2, ty2) - torch.maximum(py1, ty1), min=0))
            union = (px2 - px1) * (py2 - py1) + (tx2 - tx1) * (ty2 - ty1) - inter
            ious.append((inter / (union + 1e-7)).item())

    return (sum(ious) / len(ious), len(ious)) if ious else (0.0, 0)
...
def ntl_loss_func(
    ...
    num_items_in_batch: Optional["torch.Tensor"] = None,
    *,
    ntl_weight: float = 0.25,
    coord_lo: int = 0,
    coord_hi: int = 0,
    ignore_index: int = IGNORE_INDEX,
):
    r"""Cross-entropy plus Number Token Loss (NTL).

    ``total = CE + ntl_weight * NTL``.

    CE is the standard next-token loss over the full vocabulary. NTL acts only where the
    target token is a coordinate token in the contiguous block ``[coord_lo, coord_hi]``
    (value(id) = id - coord_lo). For each such position we take the softmax over just the
    coordinate columns (a distribution over the ordered bins 0..1000) and penalise the
    Wasserstein-1 distance to the one-hot target, which for a point-mass target equals
    ``E_p[|bin - target|]`` — the expected absolute coordinate error, normalised to [0, 1]
    by the axis length. This makes predicting 199 for target 200 cheap and 800 expensive,
    unlike plain CE.

    A differentiable geometric (DIoU) term used to live here as a third summand. It was
    removed on 2026-07-29: a clean A/B on Qwen3.5-2B under the SOTA conditions scored
    80.77 vs 80.90 bbox IoU, i.e. -0.13 with CI [-0.77, +0.50] — a tie, matching the
    literature ceiling of ~1 AP for loss-side changes. ``expected_box_iou`` survives it as
    a pure diagnostic; it never contributed to the gradient.

    Both terms are normalised by the SAME denominator so that summing micro-batch losses
    stays correct and the effective weight does not scale with
    ...
    shift_labels_2d = labels[..., 1:].contiguous().to(logits.device)  # [B, T], for the box_IoU log
    shift_labels = shift_labels_2d.view(-1)
    logits_3d = logits  # [B, T, V], for the box_IoU log
    logits = logits.view(-1, vocab_size)

    valid_mask = shift_labels != ignore_index
    ce_sum = F.cross_entropy(logits, shift_labels, ignore_index=ignore_index, reduction="sum")
    if num_items_in_batch is not None:
        denom = num_items_in_batch.to(logits.device) if torch.is_tensor(num_items_in_batch) else num_items_in_batch
        ce = ce_sum / denom
    else:
        denom = valid_mask.sum().clamp(min=1)
        ce = ce_sum / denom

    coord_mask = valid_mask & (shift_labels >= coord_lo) & (shift_labels <= coord_hi)
    if coord_mask.any():
        n_bins = coord_hi - coord_lo + 1
        coord_logits = logits[coord_mask][:, coord_lo : coord_hi + 1]  # [M, n_bins]
        probs = torch.softmax(coord_logits, dim=-1)
        bins = torch.arange(n_bins, device=logits.device, dtype=probs.dtype)
        targets = (shift_labels[coord_mask] - coord_lo).to(probs.dtype)
        dist = (bins.unsqueeze(0) - targets.unsqueeze(1)).abs() / (n_bins - 1)  # [M, n_bins] in [0, 1]
        wass = (probs * dist).sum(dim=-1)  # [M] per-coord normalized error E_p[|bin-target|]/(n_bins-1) in [0,1]
        # Normalize by the SAME denom as CE (num_items_in_batch) so summing micro-batch losses stays
        # correct and the effective weight does NOT scale with gradient_accumulation_steps. This makes
        # lambda mean "NTL gradient per coord position relative to CE's" -> lambda=1 is balanced.
        ntl = wass.sum() / denom
        mean_wass = wass.mean()  # human-readable: avg normalized coord error (bins-off / 1000)
    else:
        ntl = torch.zeros((), device=logits.device, dtype=logits.dtype)
        mean_wass = ntl

    if _NTL_DEBUG["n"] < 10 or _NTL_DEBUG["n"] % _NTL_LOG_EVERY == 0:
        n_coord = int(coord_mask.sum())
        msg = (
            f"[NTL] call~{_NTL_DEBUG['n']}: CE={ce.item():.4f}  NTL_term={ntl.item():.5f}  "
            f"lambda*NTL={ntl_weight * ntl.item():.5f}  mean_coord_err={mean_wass.item():.4f}  (coords={n_coord})"
        )
        # box_IoU is the number that says whether the boxes are actually converging, which a
        # falling total loss does not. Diagnostic only — it is computed under no_grad and is not
        # part of the returned loss.
        box_iou, n_boxes = expected_box_iou(logits_3d, shift_labels_2d, coord_lo, coord_hi)
        total = ce.item() + ntl_weight * ntl.item()
        msg += (
            f"\n[IoU] box_IoU={box_iou:.4f}  boxes={n_boxes}  "
            f"share: CE={ce.item() / total:.1%} NTL={ntl_weight * ntl.item() / total:.1%}"
        )
...
_NTL_DEBUG = {"n": 0}
# Log the first 10 calls, then every Nth. Set NTL_LOG_EVERY=1 to trace every micro-batch
# when validating a loss change.
_NTL_LOG_EVERY = max(1, int(os.environ.get("NTL_LOG_EVERY", "40")))
