...
class CoordEmbeddingGradMaskCallback(TrainerCallback):
    r"""Restrict (tied) embedding gradients to the new coordinate-token rows only.

    With ``resize_vocab`` + LoRA ``modules_to_save``, the whole embedding/LM-head becomes a
    fully trainable matrix, so gradient from the tied output head reaches every row and can
    drift the language/reasoning distribution on a small dataset (forgetting). This callback
    registers a backward hook that zeros the gradient of all rows outside ``[coord_lo,
    coord_hi]`` on each trainable embedding-shaped parameter, so only the 1001 coordinate
    rows learn while the original tokens stay frozen.
    """

    def __init__(self, coord_lo: int, coord_hi: int) -> None:
    ...
    def _make_hook(self):
        lo, hi, masks = self.coord_lo, self.coord_hi, self._masks

        def hook(grad: "torch.Tensor") -> "torch.Tensor":
            key = grad.shape[0]
            mask = masks.get(key)
            if mask is None:
                mask = torch.zeros(key, 1, device=grad.device, dtype=grad.dtype)
                mask[lo : hi + 1] = 1.0
                masks[key] = mask
    ...
    def on_train_begin(self, args: "TrainingArguments", state: "TrainerState", control: "TrainerControl", **kwargs):
        ...
        for name, param in model.named_parameters():
            ...
            if "embed_tokens" in name or "lm_head" in name:
                self._handles.append(param.register_hook(self._make_hook()))
                hooked += 1
                logger.info_rank0(f"Grad-mask on '{name}' (shape {tuple(param.shape)}); rows [{self.coord_lo},{self.coord_hi}] trainable.")
        if hooked == 0:
            logger.warning_rank0("CoordEmbeddingGradMaskCallback found no trainable embedding param to mask.")
