...
class FinetuningArguments(
    ...
    use_ntl_loss: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether to add the Number Token Loss (NTL) auxiliary term on quantized "
                "coordinate tokens. Requires a contiguous token block <coord_0>..<coord_1000>."
            )
        },
    )
    ntl_loss_weight: float = field(
        default=1.0,
        metadata={
            "help": (
                "Weight lambda of the NTL term added to cross-entropy (total = CE + lambda * NTL). "
                "NTL is the mean per-coordinate normalized error (O(1), same scale as per-token CE), "
                "so lambda ~ 1 is a balanced start; sweep 0.5/1/2 on val mIoU."
            )
        },
    )
    ntl_coord_token_prefix: str = field(
        default="<coord_",
        metadata={"help": "Prefix of the quantized coordinate tokens; ids <prefix>0..<prefix>1000 must be contiguous."},
    )
    train_only_new_token_embeddings: bool = field(
        default=False,
        metadata={
            "help": (
                "When resize_vocab is on, restrict embedding gradients to the new coordinate token "
                "rows only (freezes all original rows) to prevent forgetting of the language/reasoning "
                "distribution. Requires the <coord_0>..<coord_1000> block."
            )
        },
    )
