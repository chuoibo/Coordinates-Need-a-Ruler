...
class BaseModelArguments:
    ...
    init_special_tokens: Literal["noise_init", "desc_init", "desc_init_w_noise", "gaussian_init"] = field(
        default="noise_init",
        metadata={
            "help": (
                "Initialization method for new special tokens: "
                "'noise_init' (default, random noise around the old-vocabulary mean), "
                "'desc_init' (semantic initialization from descriptions), "
                "'desc_init_w_noise' (semantic + random noise), "
                "'gaussian_init' (pure N(0, sigma^2), no centroid -- the arm PaliGemma reports as better). "
                "Note: 'desc_init' methods require new_special_tokens_config."
            )
        },
    )
    init_special_tokens_sigma: float = field(
        default=0.02,
        metadata={
            "help": (
                "Standard deviation for 'gaussian_init'. Defaults to 0.02, the value used by "
                "PaliGemma. Use 0.015 to match the per-element std of Qwen3.5-2B's pretrained rows."
            )
        },
    )
